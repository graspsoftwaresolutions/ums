...
class CommonHelper
{
    ...
    public static function getStatusMembers($statusmonth,$status){
        $members = DB::table('mon_sub_member as mm')->select('m.name', 'm.member_number','m.gender','com.company_name','m.doj','m.employee_id',DB::raw('IF(`m`.`new_ic`="",`m`.`old_ic`,`m`.`new_ic`) as ic')
        ,DB::raw('IF(`m`.`levy`="Not Applicable","N/A",`m`.`levy`) as levy'),DB::raw('IF(`m`.`tdf`="Not Applicable","N/A",`m`.`tdf`) as tdf'),'m.tdf_amount',DB::raw('CONCAT( `com`.`short_code`, "/",  `cb`.`branch_shortcode` ) AS companycode'),'cb.branch_name as branch_name','m.salary','mm.Amount','mm.NRIC')
               ->leftjoin('mon_sub_company as mc','mc.id','=','mm.MonthlySubscriptionCompanyId')
               ->leftjoin('mon_sub as ms','ms.id','=','mc.MonthlySubscriptionId')
               ->leftjoin('membership as m','mm.MemberCode','=','m.id')
               ->leftjoin('company_branch as cb','cb.id','=','m.branch_id')
               ->leftjoin('company as com','com.id','=','mc.CompanyCode')
               ->where('ms.Date', '=', $statusmonth)
               ->where('mm.StatusId', '=', $status)
               //->dump()
               ->get();
               //->leftjoin('status as s','s.id','=','mm.StatusId')
        return $members;
    ...
    public static function getSubscriptionBankMembers($typeid,$referenceid,$fromdate,$todate){
        $memberids = CacheMonthEnd::getSubscriptionBankMembers($typeid,$referenceid,$fromdate,$todate);
        // if($typeid==1){
    ...
    public static function getLastPaidDate($memberid){
        $date = DB::table('member_payments as ms')->where('ms.member_id', '=' ,$memberid)->pluck('ms.last_paid_date')->first();
        //$duedata = DB::table('membermonthendstatus as ms')->select('ms.TOTALMONTHSDUE','ms.LASTPAYMENTDATE')->where('ms.MEMBER_CODE', '=' ,$memberid)->OrderBy('ms.StatusMonth','desc')->limit(1)->first();
        // dd($duecount);
        // $duecount = $duecount=='' ? 0 : $duecount;
        return $date;
    ...
    public static function getMonthendStatuscount($statusmonth,$status){
        $count = DB::table('membermonthendstatus as mm')
            ->select('mm.SUBSCRIPTION_AMOUNT')
            ->where('mm.StatusMonth', '=',  $statusmonth)
            ->where('mm.STATUS_CODE', '=' ,$status)
    ...
    public static function getLastPaidCurrentUnpaidCount($date,$status){
        if($date==""){
            $date = date('Y-m-01');
        }
        $month = date("m", strtotime($date));
    ...
    public static function getResignedMemberscount($date){
        $fromdate = $date;
    ...
    public static function getTotalMemberscount($date){
	 ...
	 public static function GetMonthsNewCount($frommonth,$tomonth)
    {
        $ts1 = strtotime($frommonth);
		$ts2 = strtotime($tomonth);
    ...
    public static function getMonthNewMembercount($month){
        ...
        $count = DB::table('membership as m')
            ...
            ->whereNull('m.old_member_number')
            ->count();
           // dd($count);
        return $count;
    }

    public static function getMonthRejoinMembercount($month){
        $fromdate = date('Y-m-01', strtotime($month));
        $todate = date('Y-m-t', strtotime($month));
        $count = DB::table('membership as m')
            ->select('m.id')
            ...
            ->where('m.doj', '<=', $todate)
            ->where('m.designation_id', '!=' ,3)
            ->whereNotNull('m.old_member_number')
            ->count();
           // dd($count);
        return $count;
    }

    public static function getMonthSpecialMembercount($month){
        $fromdate = date('Y-m-01', strtotime($month));
        $todate = date('Y-m-t', strtotime($month));
        $count = DB::table('membership as m')
            ->select('m.id')
    ...
    public static function getMonthTotalMembercount($month){
        $fromdate = date('Y-m-01', strtotime($month));
        $todate = date('Y-m-t', strtotime($month));
        $count = DB::table('membership as m')
            ->select('m.id')
    ...
    public  static function getMonlthlyDefaulterMembercount($statusmonth){
        $lastmonth = date('Y-m-01', strtotime('-1 months',strtotime($statusmonth)));
        $totalcount = 0;
        $membersdata = [];
        $members = DB::table('membermonthendstatus as mm')
    ...
    public  static function getMonlthlyActiveToResigncount($statusmonth){
        $lastmonth = date('Y-m-01', strtotime('-1 months',strtotime($statusmonth)));
        $totalcount = 0;
        $membersdata = [];
        $members = DB::table('membermonthendstatus as mm')
    ...
    public  static function getMonlthlyDefaultToActivecount($statusmonth){
        $lastmonth = date('Y-m-01', strtotime('-1 months',strtotime($statusmonth)));
        $totalcount = 0;
        $membersdata = [];
        $members = DB::table('membermonthendstatus as mm')
    ...
    public  static function getMonlthlyStruckoffToActivecount($statusmonth){
        $lastmonth = date('Y-m-01', strtotime('-1 months',strtotime($statusmonth)));
        $totalcount = 0;
        $membersdata = [];
        $members = DB::table('membermonthendstatus as mm')
