...
<div class="row">
			...
			<div class="card-content">
				...
				<form method="post" id="filtersubmit" action="">
					...
					<div class="row">                          
						...
						<div class="col s12 m6 l2 @if($user_role !='union') hide @endif">
						...
						<div class="col s12 m6 l2">
							...
							<select name="status_id" id="status_id" class="error browser-default selectpicker" data-error=".errorTxt23" >
								<option value="">{{__('Active & Defaulter') }}</option>
								<option value="1">{{__('Active') }}</option>
								<option value="2">{{__('Defaulter') }}</option>
	...
	<div class="row">
		<div class="col s12">
			<iframe src="{{ url(app()->getLocale().'/get-pgm-members-report') }}?month_year={{date('M/Y')}}&company_id=&branch_id=&member_auto_id=&unionbranch_id=&from_member_no=&to_member_no=&status_id=" id="myframe" height="500px" width="100%"></iframe>
			<!-- <div class="row">
				<div class="col s12">
					<ul class="tabs">
...
$("#member_pgm_sidebar_li_id").addClass('active');
$("#member_pgm_sidebar_a_id").addClass('active');
	...
	$(document).on('submit','form#filtersubmit',function(event){
		...
		if(month_year!="" || company_id!="" || branch_id!="" || member_auto_id!=""){
			var searchfilters = '&month_year='+month_year+'&company_id='+company_id+'&branch_id='+branch_id+'&member_auto_id='+member_auto_id+'&unionbranch_id='+unionbranch_id+'&from_member_no='+from_member_no+'&to_member_no='+to_member_no+'&status_id='+status_id;
			
			$("#myframe").contents().find("html").css('opacity',0);
			$("#myframe").attr("src", "{{ url(app()->getLocale().'/get-pgm-members-report') }}?offset=0"+searchfilters,);
