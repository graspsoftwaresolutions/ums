                        ...
                        <div class="col s12">
                            <div class="card">
                                <div class="card-content">
                                    <h4 class="card-title ">{{ __('Edit Form Type') }}</h4>
                                    @include('includes.messages')
                                   <div id="view-validations">  
                                   {{$row=''}}
                                   @php ($row = $data['form_edit'])
                                    <form class="formValidate" id="FormTypeformValidate" method="post" action="{{ route('formtype.update',[app()->getLocale(),$row->id]) }}">
                                       @method('PATCH')
                                        @csrf
                                      <div class="row">
                                        <div class="input-field col s12 m6">
                                          <label for="formname" class="common-label">{{__('Form Name') }}*</label>
                                          <input id="formname" class="common-input" value="{{$row->formname}}" name="formname" type="text" data-error=".errorTxt1">
                                          <div class="errorTxt1"></div>
                                        </div>
                                        <div class="input-field col s12 m6">
                                          <label for="orderno" class="common-label">{{__('Order No') }}*</label>
                                          <input id="orderno" class="common-input" value="{{$row->orderno}}"  name="orderno" type="text" data-error=".errorTxt1">
                                          <div class="errorTxt1"></div>
                                        </div>
                                        <div class="input-field col s12 m6">
                                        <select class="error browser-default" class="common-select" id="status" name="status"  data-error=".errorTxt2">
