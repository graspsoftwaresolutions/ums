							...
							<div class="container">
								<div class="row">
									...
									<div class="col s2 m6 l6 ">
										<a class="btn waves-effect waves-light breadcrumbs-btn right" href="{{ route('master.addrace', app()->getLocale())  }}">{{__('Add New Race') }}</a>
                           ...
                           @foreach($data['race_view'] as $key=>$value)
                            ...
                            $parameter = Crypt::encrypt($parameter);    
													...
													@endphp
														...
														<a class="btn-small waves-effect waves-light cyan" href="{{ route('master.editrace', [app()->getLocale(), $parameter]) }}">{{__('Edit') }}</a>
														<a class="btn-small waves-effect waves-light amber darken-4" href="{{ route('master.deleterace',[app()->getLocale(),$parameter])}}" onclick="if (confirm('{{ $confirmAlert }}')) return true; else return false;">{{__('Delete') }}</a></td>
												  </tr>
												  @endforeach
												</tbody>
