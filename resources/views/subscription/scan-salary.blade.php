                                ...
                                <div class="card-content">
									...
									<div class="row">
										<div class="col offset-s2 s8">
											<h5 class="">{{__('Updating Salary details, please dont refresh the page....') }}</h5>
...
<script>
	...
	$(document).ready(function() {
		...
		var monthstring = {{ $data['monthstring'] }};
		var inctype = {{ $data['inctype'] }};
	...
	function ScanSalary(monthstring,inctype,start){
		var lastrow ={{$lastrow}};
		var limit = {{ $limit }};
		if(monthstring!=""){
		var url = "{{ url(app()->getLocale().'/salary-scanning') }}" + '?monthstring=' + monthstring+ '&inctype='+inctype+ '&limit='+limit+ '&start='+start;
			$.ajax({
				url: url,
				type: "GET",
				dataType: 'JSON',
				success: function(result) {
					...
					if(result.status==1){
						...
						if(start!=lastrow){
							...
							//progress_count = progress_count>limit ? limit : progress_count;
							$('#check_updated_'+start+' span').html('<i class="material-icons">done</i>');
							ScanSalary(monthstring,inctype,parseInt(start+limit));
