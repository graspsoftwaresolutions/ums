...
<form class="formValidate" id="addarrear_formValidate" method="post"
  action="{{ route('subscription.saveBulkArrear',app()->getLocale()) }}">
  @csrf
  <div class="container">
    <h5 class="padding-left-10"> Bulk Arrear Entry Details <a class="btn waves-effect waves-light right" href="{{ route('subscription.arrearentry',app()->getLocale())  }}">{{__('Back') }}</a></h5>
    <div class="row">
      ...
      <div class="col s12 m6">
        <label
          class="common-label  force-active">{{__('No of Rows') }}*</label>
        <input id="no_of_rows" class="common-input"
          name="no_of_rows" type="text" data-error=".errorTxt7">
            <input id="no_of_months" class="common-input hide"
...
messages: {
  ...
  no_of_rows : {
    required: '{{__("Please Enter No of Rows") }}',
    digits: '{{__("please Enter numbers only")}}'
  }
},
errorElement: 'div',
