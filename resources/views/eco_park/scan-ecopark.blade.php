...
<script>
	...
	$(document).ready(function() {
		...
		if(row_count>0){
			setTimeout(function(){
			  ...
			  ScanMembership(0,{{ $data['park_auto_id'] }},{{$limit}});
			}, 1500);
		}else{
	...
	function ScanMembership(start,parkautoid,limitval){
		var lastrow ={{$lastrow}};
		
		var url = "{{ url(app()->getLocale().'/ecopark-scanning') }}" + '?start='+start+'&park_auto_id='+parkautoid+'&limit='+limitval;
			$.ajax({
				url: url,
				type: "GET",
				dataType: 'JSON',
				success: function(result) {
					...
					if(result.status==1){
						...
						if(start!=lastrow){
							...
							ScanMembership(parseInt(start+limit),parkautoid,limitval);
